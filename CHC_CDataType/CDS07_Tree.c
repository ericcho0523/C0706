...
// 🍚 전위 순회로 모든 노드 출력하기 -> 나중에검색 🍚
void PrintPre(Node* current){
    // 1. 내가 비어있으면 끝나야 함
    if(current == NULL) return;
    // 2. 안비어있으면 전위순회니까 출력먼저
...
// 🦪 Pre/In/Post중 선택해서, 원하는값 있는지 검색하기->index 🦪
void Search(Node* start, int target){
    // 1. 어떤 방식으로 검색할지 물어보기 -> 귀찮으니 결정
    int searchType = 2; // 1 Pre, 2 In 3 Post
    // 2. 검색 구현
    // 단, 전역함수인 cnt와 find를 반드시 초기화 해 줘야 함.
    cnt  = 0;
    find = 0;

    if(searchType == 1){
        SearchPre(start, target); // index위치는 얘가 알려줌
        // 몇번만에 찾았는지, 못 찾았는지 구현
        if(find==0){
            // 못 찾음... -> 몇번만에 못 찾았는지 알려주기
...
// ----------------------------------------------------
// ❤️❤️❤️ 이진 탐색 트리(Binary Search Tree) ❤️❤️❤️
// 트리를 효율적으로 관리하기 위한 알고리즘
// 수동으로 트리에 데이터를 넣을 경우, 매우 비 효율적임
// ----------------------------------------------------
void InsertBST(Node** mainHead, int data){
    ...
    // 2. 비어있지 않다면, 나(data)다 비교하는놈이 더 크면, 나 왼쪽
...
/*
6일차 숙제
이진탐색트리를 이용한 트리 관리 시스템을 만들어주세요
아래의 기능은 반드시 포함되어 있어야 합니다.
1. 트리를 출력하기
   -> 1 전위 2 중위 3 후위 물어본 후 출력하기
      + 만약, 트리가 처음부터 비어있으면, 아예 물어보지도 말기 
2. 데이터를 삽입하기
   -> 데이터 입력받고 BST 알고리즘을 이용해서 추가하기
3. 트리 초기화하기  
4. 검색하기
   -> 1 전위 2 중위 3 후위 물어본 후 Search 진행하기
5. 종료하기     
